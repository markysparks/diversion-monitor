...
class MetarTafDialog(dialog_framework.Dialog):
    """Tools to display METAR/TAF messages in a window box using the dialog framework"""
    def body(self, master):
        ttk.Label(master, text="METAR/SPECI:").grid(row=0, sticky=tk.W, pady=5)
        ttk.Label(master, text="TAF:").grid(row=2, sticky=tk.W, pady=5)

        self.metar_msg_field = tk.Text(master, height=3, wrap="word", background="cyan", width=75)
        ...
        # return self.e1  # initial focus if required
